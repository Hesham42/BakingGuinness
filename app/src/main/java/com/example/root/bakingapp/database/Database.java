...
public class Database extends SQLiteOpenHelper {
    ...
    public Database(Context context) {
        super(context, Contract.DATABASE_NAME,
                null, 1);
    ...
    public void insertItem(WidgetModel model, int widgetId) {
        ...
        for (int i = 0; i < model.ingredients.size(); i++) {
            ...
            values2.put(Contract.COLUMN_QUANTITY, Double.valueOf(((Ingredient) model.ingredients.get(i)).getQuantity()));
    ...
    public ArrayList<Ingredient> getIngredients(int widgetId) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                ingredient.setQuantity(cursor.getDouble(cursor.getColumnIndex(Contract.COLUMN_QUANTITY)));
                list.add(ingredient);
            } while (cursor.moveToNext());
        }
        return list;
